...
**Learning:** small **`.sv0`** sources (numbered tutorials through **`16_try_operator.sv0`**) and commands for **`vm-compile`** / **`vm-run`** / **`emit-c`** are under [`sv0c/examples/learn/`](sv0c/examples/learn/README.md). **`vm-compile`** and **`emit-c`** take paths relative to **`sv0c/`**; **`vm-run`** accepts **`sv0c/build/vm/<stem>.sv0b`** relative to this **meta-repo root** (or an absolute path).
...
| **sv0c commit pinned on this branch (`main`)** | `8ff93e31eefb2dffdfea11ed0429e7630cf3f2cc` |
