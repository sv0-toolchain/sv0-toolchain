...
if [[ -f "$SRC_DIR/name_resolution/sources.cm" ]]; then
    cd "$SV0C_ROOT" && echo 'CM.make "sml-legacy/name_resolution/sources.cm";' | sml 2>&1
    echo "OK: resolver module compiles"
elif [[ -f "$SV0C_ROOT/sources.cm" ]]; then
    cd "$SV0C_ROOT" && echo 'CM.make "sources.cm";' | sml 2>&1
    echo "OK: sv0c compiles (root sources.cm; no sml-legacy/name_resolution/sources.cm yet)"
else
